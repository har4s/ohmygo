...
func TestNewRunner(t *testing.T) {
	...
	expectedQueries := []string{
		"CREATE TABLE IF NOT EXISTS `migrations` (file VARCHAR(255) PRIMARY KEY NOT NULL, applied INTEGER NOT NULL)",
	}
...
func createTestDB() (*testDB, error) {
	sqlDB, err := sql.Open("mysql", "har4s:@/ozzo_dbx_test?parseTime=true")
	if err != nil {
		return nil, err
	}

	db := testDB{DB: dbx.NewFromDB(sqlDB, "mysql")}
	db.DB.QueryLogFunc = func(ctx context.Context, t time.Duration, sql string, rows *sql.Rows, err error) {
		db.CalledQueries = append(db.CalledQueries, sql)
	}
	db.DB.ExecLogFunc = func(ctx context.Context, t time.Duration, sql string, result sql.Result, err error) {
		db.CalledQueries = append(db.CalledQueries, sql)
	}
